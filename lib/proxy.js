    ...
    var onResponse = function(err, response) {
      ...
      if (!err && response.statusCode === 200) {
        if ((req.headers['accept'] || '').toLowerCase() === 'application/json') {
          // This is a metadata request, don't cache it
          return respondWithoutCache(dest, r, req, res);
        }
...
function respondWithoutCache(dest, r, req, res) {
  var log = exports.log,
      contentLength = r.response.headers['content-length'],
      contentType = r.response.headers['content-type'];

  log.info('metadata', dest);
  log.debug('size: %s, type: "%s"', contentLength, contentType);
  res.setHeader('Content-Length', contentLength);
  res.setHeader('Content-Type', contentType);
  res.setHeader('Connection', 'keep-alive');
  res.setHeader('X-Cache-Hit', 'false');
  return r.pipe(res, { end: false });
}
