  ...
  if (match && match[1] && match[2]) {
    ...
    if (!fs.existsSync(fullPath)) {
      ...
      try {
        ...
        log.info("Unable to get data for " + cachePath.full + " from npm. Tgz cannot be verified, so it will be downloaded.")
        log.debug("Error is: ", err);
        // Consider returning true and leave the client's npm to verify the .tgz.
        return false;
      }  
    } else {
